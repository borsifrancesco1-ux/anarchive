...
import { Icon, AnarchiveMark, Kbd, KindBadge } from './components.jsx';
import { formatAgo } from './utils.js';

export function MenuBarView({ clips, onOpenFull, onOpenSettings, onQuit }) {
  ...
  return (
    <div data-screen-label="03 Menu bar"
      ...
      <div style={{
        ...
        overflow: 'hidden', border: '0.5px solid #2e2440',
      }}>
        {/* Header */}
        <div style={{
          padding: '10px 12px', display: 'flex', alignItems: 'center', gap: 8,
          borderBottom: '1px solid #2e2440', background: '#15101c',
        }}>
          ...
          <input
            ...
            style={{
              ...
              fontFamily: 'var(--ac-font-ui)', fontSize: 13, color: '#f3eef9',
            }}
          />
          <Kbd dark>Alt Space</Kbd>
        </div>

        {/* Tabs */}
        <div style={{ display: 'flex', gap: 4, padding: '8px 8px 4px', borderBottom: '1px solid #2e2440' }}>
          {[
        ...
        <div style={{ maxHeight: 320, overflowY: 'auto', padding: 6 }}>
          {list.length === 0 ? (
            ...
            <MenuClipRow key={clip.id} clip={clip} idx={i}
              onClick={() => {
                navigator.clipboard?.writeText(clip.text).catch(() => {});
                window.anarchive?.window?.close();
              }} />
          ))}
        </div>

        {/* Footer */}
        <div style={{ display: 'flex', gap: 2, padding: 6, borderTop: '1px solid #2e2440', background: '#15101c' }}>
...
function MenuClipRow({ clip, idx, onClick }) {
  ...
  return (
    <div
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontFamily: 'var(--ac-font-mono)', fontSize: 9.5, color: '#80738f', marginTop: 1 }}>
          {formatAgo(clip.at)}{clip.app ? ' · ' + clip.app : ''}
        </div>
      </div>
      <span style={{ fontFamily: 'var(--ac-font-mono)', fontSize: 9.5, color: '#80738f', minWidth: 10, textAlign: 'right' }}>
        {idx < 9 ? '⌘' + (idx + 1) : ''}
      </span>
    </div>
  );
  ...
  return (
    <button onClick={onClick}
      ...
      style={{
        ...
        color: hover ? '#FF8A33' : '#b9accd',
        border: '1px solid ' + (hover ? '#FF8A33' : 'transparent'),
        borderRadius: 6,
        fontFamily: 'inherit', fontSize: 11.5, fontWeight: 500, cursor: 'pointer',
        transform: hover ? 'scale(1.04)' : 'scale(1)',
        boxShadow: hover ? '0 0 0 1px rgba(255,138,51,0.40), 0 0 10px 0 rgba(255,138,51,0.30)' : 'none',
        transition: 'transform 180ms cubic-bezier(0.4, 0, 0.2, 1), box-shadow 180ms cubic-bezier(0.4, 0, 0.2, 1), background 180ms ease, color 180ms ease, border-color 180ms ease',
      }}>
